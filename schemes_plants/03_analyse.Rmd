...
In een eerste stap berekenen we de populatiegrootte per meetnetlocatie voor elk jaar (waarin er een telling is gebeurd).
...
check_missing_cyclus <- total_cyclus %>%
  filter(!cyclus_1_2) %>%
  select(meetnet, locatie, protocol, cyclus, jaar, tot_mean, variable) %>%
  pivot_wider(names_from = "cyclus", values_from = c("tot_mean", "jaar"))
...
missing_cyclus_1 <- check_missing_cyclus %>%
  ...
  mutate(cyclus = 1,
         ...
         jaar = ifelse(!is.na(jaar_0) & (jaar_0 == 2015), jaar_0,
                           jaar_2)) %>%
  select(meetnet, locatie, protocol, cyclus, periode, jaar, tot_mean, variable)
...
*Opmerking locatie Rechterscheldeoever Uitbergen-Schoonaarde van meetnet Driekantige bies is op inactief gezet in 2021, met als opmerking 'Hier werden dijkwerken uitgevoerd en werden de planten getransloceerd door INBO'. Nultelling voor periode 2019 - 2021??*
...
Een overzicht van de bijpassingen vind je in onderstaande tabel.
*Niet in eindrapport*
